import {
  Box,
  Grid,
  GridItem,
  HStack,
  Image,
  Text,
  VStack,
} from '@chakra-ui/react';

import {
  AiFillFacebook,
  AiFillInstagram,
  AiFillTwitterSquare,
  AiFillYoutube,
} from 'react-icons/ai';
import { FaPinterest, FaSnapchatGhost, FaTumblr } from 'react-icons/fa';
export default function Footer() {
  const Gridmap = [
    { icon: <AiFillFacebook /> },
    { icon: <AiFillInstagram /> },
    { icon: <AiFillTwitterSquare /> },
    { icon: <FaPinterest /> },
    { icon: <FaSnapchatGhost /> },
    { icon: <FaTumblr /> },
    { icon: <AiFillYoutube /> },
  ];
  return (
    <HStack
      w={'full'}
      bg={'gray.800'}
      h={'300px'}
      py={20}
      justifyContent={'center'}
      alignItems={'flex-start'}
    >
      <Grid w="7xl" gap={4} templateColumns={'1fr 1fr 1fr 1fr'}>
        {/* first */}
        <GridItem>
          <Box w={32}>
            <Image
              src="data:image/png;base64,"
              alt="marvel"
            />
          </Box>
        </GridItem>
        {/* second */}
        <GridItem w={'full'}>
          <HStack w={'full'} spacing={8}>
            <VStack
              color={'gray.100'}
              fontWeight={600}
              alignItems={'flex-start'}
              textTransform={'uppercase'}
            >
              <Text border={'1px solid'}>About marvel</Text>
              <Text>Help/faqs</Text>
              <Text>careers</Text>
              <Text>internships</Text>
            </VStack>
            <VStack
              color={'gray.400'}
              fontWeight={600}
              alignItems={'flex-start'}
              textTransform={'uppercase'}
            >
              <Text>advertizing</Text>
              <Text>disney+</Text>
              <Text>marvelhq.com</Text>
              <Text>redeem digital</Text>
            </VStack>
          </HStack>
        </GridItem>
        {/* third */}
        <GridItem w={'full'}>
          <VStack spacing={6}>
            <HStack spacing={6}>
              <Box w={16}>
                <Image
                  src="https://images.unsplash.com/photo-1531907700752-62799b2a3e84?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NHx8aGVyb3xlbnwwfHwwfHx8MA%3D%3D&auto=format&fit=crop&w=500&q=60"
                  alt="what"
                />
              </Box>
              <VStack alignItems={'flex-start'} lineHeight={4}>
                <Text fontWeight={600} color={'gray.100'}>
                  hero
                </Text>
                <Text color={'gray.400'}>we always want to be hero</Text>
              </VStack>
            </HStack>
            <HStack spacing={6}>
              <Box w={16}>
                <Image
                  src="https://images.unsplash.com/photo-1501432377862-3d0432b87a14?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8OXx8aGVyb3xlbnwwfHwwfHx8MA%3D%3D&auto=format&fit=crop&w=500&q=60"
                  alt="what"
                />
              </Box>
              <VStack alignItems={'flex-start'} lineHeight={4}>
                <Text fontWeight={600} color={'gray.100'}>
                  deadpool
                </Text>
                <Text color={'gray.400'}>great fool hero</Text>
              </VStack>
            </HStack>
          </VStack>
        </GridItem>
        <GridItem w={'full'}>
          <VStack alignItems={'flex-start'} spacing={4}>
            <Text
              fontWeight={600}
              textTransform={'uppercase'}
              color={'gray.100'}
            >
              follow marvel
            </Text>
            <Grid
              w={'full'}
              templateColumns={'repeat(4,1fr)'}
              color={'gray.100'}
              gap={6}
              fontSize={'20px'}
            >
              {Gridmap.map((item, i) => {
                return <GridItem key={i}>{item.icon}</GridItem>;
              })}
            </Grid>
          </VStack>
        </GridItem>
      </Grid>
    </HStack>
  );
}
